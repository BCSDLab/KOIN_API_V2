package in.koreatech.koin.socket.session.repository;
...
public class WebSocketUserSessionRedisRepository {
    ...
    public Optional<WebSocketUserSession> findUserSession(Integer userId) {
        try {
            String key = createKey(userId);
    ...
    private String serialize(WebSocketUserSession value) {
        try {
    ...
    private WebSocketUserSession deserialize(Object value) {
        if (value == null) return null;
        try {
            return objectMapper.readValue((String) value, WebSocketUserSession.class);
        } catch (JsonProcessingException e) {
