...
class UserVerificationStatusTest {
    ...
    private UserVerificationStatus Email_인증;
    ...
    void init() {
        ...
        Email_인증 = UserVerificationStatusFixture.Email_인증(TEST_EMAIL, CORRECT_CODE);
    }
    ...
    class verify {
        ...
        void verify를_이용하여_Email_인증용_객체를_인증할_수_있다() {
            ...
            // when
            Email_인증.verify(CORRECT_CODE);

            // then
            assertThat(Email_인증.isVerified()).isTrue();
            assertThat(Email_인증.getExpiration()).isEqualTo(expectedExpiration);
        }
        ...
        void Email_인증용_객체에_잘못된_인증번호를_입력하면_KoinIllegalArgumentException이_발생한다() {
            // when / then
            assertThatThrownBy(() -> Email_인증.verify(WRONG_CODE))
                .isInstanceOf(KoinIllegalArgumentException.class)
                .hasMessage("인증 번호가 일치하지 않습니다.");
        }
    }
    ...
    class requireVerified {
        ...
        void Email_인증된_상태에서_requireVerified를_호출하면_예외가_발생하지_않는다() {
            ...
            // when / then
            assertThatCode(Email_인증::requireVerified)
                .doesNotThrowAnyException();
        }
        ...
        void Email_미인증_상태에서_requireVerified를_호출하면_AuthorizationException이_발생한다() {
            // when / then
            assertThatThrownBy(Email_인증::requireVerified)
                .isInstanceOf(AuthorizationException.class)
                .hasMessage("본인 인증 후 다시 시도해주십시오.");
        }
    }
